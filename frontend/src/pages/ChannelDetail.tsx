...
import { getChannelVideos, fetchChannelVideos, extractTranscriptsStream, prepareAllAudio, downloadPreparedAudio, exportMarkdown } from '../api/videos';
...
export default function ChannelDetail() {
  ...
  const handleSelectAll = useCallback(() => {
    if (activeTab === 'pending') {
      ...
      const extractedIds = videosByTab.extracted.map(v => v.id);
      setSelectedVideoIds(new Set(extractedIds));
    }
  }, [activeTab, videosByTab.pending, videosByTab.extracted]);
  ...
  // Count of selected extracted videos
  const selectedExtractedCount = useMemo(() => {
    const extractedIds = new Set(videosByTab.extracted.map(v => v.id));
    return Array.from(selectedVideoIds).filter(id => extractedIds.has(id)).length;
  }, [selectedVideoIds, videosByTab.extracted]);
  ...
  const handleExportMarkdown = useCallback(async () => {
    ...
    try {
      const extractedIds = new Set(videosByTab.extracted.map(v => v.id));
      const selectedIds = Array.from(selectedVideoIds).filter(id => extractedIds.has(id));
      await exportMarkdown(id, selectedIds.length > 0 ? selectedIds : undefined);
  ...
  }, [id, selectedVideoIds, videosByTab.extracted]);
    ...
    <div className="p-6">
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="flex gap-8">
          <button
            onClick={() => { setActiveTab('pending'); setSelectedVideoIds(new Set()); }}
          ...
          <button
            onClick={() => { setActiveTab('extracted'); setSelectedVideoIds(new Set()); }}
            className={`pb-3 px-1 border-b-2 font-medium text-sm transition-colors ${
              activeTab === 'extracted'
      ...
      {/* Export Actions (only show in Extracted tab) */}
      {activeTab === 'extracted' && videosByTab.extracted.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-4 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-3">
              <span className="w-8 h-8 flex items-center justify-center bg-emerald-100 text-emerald-700 text-xs font-bold rounded-lg flex-shrink-0">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <div className="text-sm text-gray-700">
                  {selectedExtractedCount > 0 ? (
                    <>Export <span className="font-semibold">{selectedExtractedCount}</span> selected video{selectedExtractedCount !== 1 ? 's' : ''} as Markdown</>
                  ) : (
                    <>Export all <span className="font-semibold">{videosByTab.extracted.length}</span> transcripts as Markdown</>
                  )}
                </div>
                <div className="text-xs text-gray-400">Video details and transcripts combined into a single .md file</div>
              </div>
            </div>
            <button
              onClick={handleExportMarkdown}
              disabled={isExporting}
              className="flex items-center gap-1.5 px-4 py-1.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50 whitespace-nowrap cursor-pointer disabled:cursor-not-allowed"
            >
              {isExporting ? (
                <>
                  <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                ...
                selectedExtractedCount > 0 ? `Export ${selectedExtractedCount}` : 'Export All'
              )}
            </button>
          </div>
        </div>
            ...
            <>
              <label className="flex items-center gap-2 cursor-pointer px-2 py-1 hover:bg-gray-50 rounded-lg" title="Select all">
                <input
                  ...
                  checked={
                    activeTab === 'pending'
                      ? selectedPendingCount === videosByTab.pending.length && videosByTab.pending.length > 0
                      : selectedExtractedCount === videosByTab.extracted.length && videosByTab.extracted.length > 0
                  }
                  ref={(el) => {
                    if (el) {
                      const count = activeTab === 'pending' ? selectedPendingCount : selectedExtractedCount;
                      const total = activeTab === 'pending' ? videosByTab.pending.length : videosByTab.extracted.length;
                ...
                />
                {(activeTab === 'pending' ? selectedPendingCount : selectedExtractedCount) > 0 && (
                  <span className="text-sm text-blue-600 whitespace-nowrap">
                    {activeTab === 'pending' ? selectedPendingCount : selectedExtractedCount}
                  </span>
                )}
              </label>
